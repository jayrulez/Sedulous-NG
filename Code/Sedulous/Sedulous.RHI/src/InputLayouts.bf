...
public class InputLayouts : IEquatable<InputLayouts>
{
	...
	/// </summary>
	/// <param name="semantic">The element semantic.</param>
	/// <param name="elements">The elements description.</param>
	/// <param name="vertexBufferIndices">The vertex buffer indices.</param>
	/// <returns>True if the input layout contains elements with the specified semantic. False otherwise.</returns>
	public bool FindLayoutElementsByUsage(ElementSemanticType semantic, List<ElementDescription> elements, List<int32> vertexBufferIndices)
	{
		List<ElementDescription> elementsList = scope List<ElementDescription>();
		List<int32> vertexBufferIndicesList = scope List<int32>();
		for (int32 i = 0; i < LayoutElements.Count; i++)
		{
			LayoutDescription layoutDescription = LayoutElements[i];
			for (int32 j = 0; j < layoutDescription.Elements.Count; j++)
			{
				...
				if (element.Semantic == semantic)
				{
					...
					vertexBufferIndicesList.Add(i);
				}
			}
		}
		elements.AddRange(elementsList);
		vertexBufferIndices.AddRange(vertexBufferIndicesList);
