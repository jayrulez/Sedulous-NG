...
public class VKFrameBuffer : VKFrameBufferBase
{
		...
		VkImageViewCreateInfo imageViewInfo = VkImageViewCreateInfo()
			{
				...
				format = vkTexture.Description.Format.ToVulkan(depthTexture),
	...
	public override void TransitionToFinalLayout(VkCommandBuffer cb)
	{
		for (int i = 0; i < ColorTargets.Count; i++)
		{
			...
			if ((texture.Description.Flags & TextureFlags.ShaderResource) != TextureFlags.None)
			{
		...
		if (DepthStencilTarget.HasValue)
		{
			...
			if ((depthTexture.Description.Flags & TextureFlags.ShaderResource) != TextureFlags.None)
			{
