...
class TestJob : Job
{
    ...
    protected override void OnExecute()
    {
        ...
        if (ShouldFail)
        {
            // Simulate job failure by canceling itself
            mState = .Canceled;
            return;
        }
...
class ResultJob<T> : Job<T>
{
    private T mValue;

    public this(StringView name, T value) : base(name, .AutoRelease)
    {
        mValue = value;
    }
...
// Unit test class using Beef's testing framework
class JobSystemTests
{
    private ILogger mLogger = new ConsoleLogger(.Trace) ~ delete _;
    ...
    public static void MainThreadJobs()
    {
        ...
        {
            jobSystem.Update(16); // Simulate 16ms frame time
            Thread.Sleep(10);
        }
    ...
    {
        var logger = scope ConsoleLogger(.Trace);
        var jobSystem = scope JobSystem(logger, 1);
        jobSystem.Startup();

        var job1 = scope TestJob("Job1");
		defer job1.ReleaseRefNoDelete();
        job1.SleepTimeMs = 100; // Make it take some time
        var job2 = scope TestJob("Job2");
		defer job2.ReleaseRefNoDelete();
        job2.AddDependency(job1);
        ...
        Test.Assert(job2.State == .Canceled); // Should be canceled due to dependency
    ...
    {
        var logger = scope ConsoleLogger(.Trace);
        var jobSystem = scope JobSystem(logger, 4);
        jobSystem.Startup();
        defer jobSystem.Shutdown();
    ...
    {
        var logger = scope ConsoleLogger(.Trace);
        var jobSystem = scope JobSystem(logger, 0);
        jobSystem.Startup();
        defer jobSystem.Shutdown();
    ...
    public static void JobSelfDependencyPrevention()
    {
        ...
        // This should trigger a Runtime.FatalError
        // We can't catch it in Beef, but we can verify the check exists
        // by inspecting the source code logic
        
        // The actual test would be: job.AddDependency(job);
        // But this would terminate the program
        
        // Instead, verify the job doesn't think it depends on itself initially
    ...
    {
        var logger = scope ConsoleLogger(.Trace);
        var jobSystem = scope JobSystem(logger, 1); // Single worker for deterministic order
        jobSystem.Startup();

        List<String> executionOrder = new .();
        var monitor = scope Monitor();

        var job1 = scope DelegateJob(scope () => { using (monitor.Enter())
            {
                executionOrder.Add(new String("First"));
            } }, false, "First", .None);
		defer job1.ReleaseRefNoDelete();

        var job2 = scope DelegateJob(scope () => { using (monitor.Enter())
            {
                executionOrder.Add(new String("Second"));
            } }, false, "Second", .None);
		defer job2.ReleaseRefNoDelete();

        var job3 =scope DelegateJob(scope () => { using (monitor.Enter())
            {
