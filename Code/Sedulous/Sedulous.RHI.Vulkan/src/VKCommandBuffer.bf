...
public class VKCommandBuffer : CommandBuffer
{
	...
	private VKMeshShaderPipelineState currentMeshShaderPipelineState;
	...
	public this(VKGraphicsContext context, VKCommandQueue queue)
	{
		...
		case CommandQueueType.Graphics:
			poolInfo.queueFamilyIndex = this.context.QueueIndices.GraphicsFamily;
			break;
		case CommandQueueType.Compute:
			poolInfo.queueFamilyIndex = this.context.QueueIndices.ComputeFamily;
			break;
		case CommandQueueType.Copy:
			poolInfo.queueFamilyIndex = this.context.QueueIndices.CopyFamily;
			break;
		}
		...
		VkRenderPassBeginInfo renderPassBeginInfo = VkRenderPassBeginInfo()
			{
				...
				renderArea = VkRect2D(frameBuffer.Width, frameBuffer.Height)
			};
				...
				VkClearRect clearRect = VkClearRect()
					{
						baseArrayLayer = 0,
						layerCount = 1,
						rect = VkRect2D(0, 0, colorTexture.Description.Width, colorTexture.Description.Height)
					};
			...
			VkClearRect clearRect = VkClearRect()
				{
					baseArrayLayer = 0,
					layerCount = 1,
					rect = VkRect2D(0, 0, depthTexture.Description.Width, depthTexture.Description.Height)
				};
			VulkanNative.vkCmdClearAttachments(CommandBuffer, 1, &clearAttachment, 1, &clearRect);
		}
	}
	...
	/// <inheritdoc />
	protected override void SetMeshShaderPipelineStateInternal(MeshShaderPipelineState pipeline)
	{
		VKMeshShaderPipelineState newPipeline = pipeline as VKMeshShaderPipelineState;
		VulkanNative.vkCmdBindPipeline(CommandBuffer, VkPipelineBindPoint.VK_PIPELINE_BIND_POINT_GRAPHICS, newPipeline.NativePipeline);
		currentMeshShaderPipelineState = newPipeline;
		activePipelineState = newPipeline;
	}
	...
	protected override void SetResourceSetInternal(ResourceSet resourceSet, uint32 index, uint32[] offsets)
	{
		...
		if (vkResourceSet.DynamicBufferCount != 0 && offsets != null)
		{
			...
			{
				for (uint32 i = 0; i < vkResourceSet.DynamicBufferCount; i++)
				{
	...
	/// <inheritdoc />
	protected override void SetVertexBuffersInternal(Sedulous.RHI.Buffer[] buffers, uint32[] offsets)
	{
		...
		for (int i = 0; i < buffers.Count; i++)
		{
			...
			vertexOffsets[i] = ((offsets != null) ? offsets[i] : 0);
		}
	...
	public override void GenerateMipmaps(Texture texture)
	{
		...
		if ((vkFormatProperties.optimalTilingFeatures & VkFormatFeatureFlags.VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_CUBIC_BIT_EXT) != VkFormatFeatureFlags.None)
		{
			filter = VkFilter.VK_FILTER_CUBIC_EXT;
		}
		else if ((vkFormatProperties.optimalTilingFeatures & VkFormatFeatureFlags.VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT) != VkFormatFeatureFlags.None)
		{
	...
	{
		VKTexture nativeSource = source as VKTexture;
		VKTexture nativeDestination = destination as VKTexture;
		VkImageAspectFlags aspectFlags = (((source.Description.Flags & TextureFlags.DepthStencil) != TextureFlags.DepthStencil) ? VkImageAspectFlags.VK_IMAGE_ASPECT_COLOR_BIT : (VkImageAspectFlags.VK_IMAGE_ASPECT_DEPTH_BIT | VkImageAspectFlags.VK_IMAGE_ASPECT_STENCIL_BIT));
				...
				extent = VkExtent3D()
					{
						...
						depth = source.Description.Depth
					},
		...
		nativeSource.TransitionImageLayout(CommandBuffer, VkImageLayout.VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 0, 1, 0, 1);
		nativeDestination.TransitionImageLayout(CommandBuffer, VkImageLayout.VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 0, 1, 0, 1);
		VulkanNative.vkCmdResolveImage(CommandBuffer, nativeSource.NativeImage, VkImageLayout.VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, nativeDestination.NativeImage, VkImageLayout.VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
		if ((nativeDestination.Description.Flags & TextureFlags.ShaderResource) != TextureFlags.None)
		{
			nativeDestination.TransitionImageLayout(CommandBuffer, VkImageLayout.VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 0, 1, 0, 1);
		}
	}
			...
			VkDebugUtilsLabelEXT labelInfo = VkDebugUtilsLabelEXT()
				{
					...
					pLabelName = scope String(label).CStr()
				};
	...
	/// <inheritdoc />
	public override BottomLevelAS BuildRaytracingAccelerationStructure(in BottomLevelASDescription description)
	{
		return new VKBottomLevelAS(context, CommandBuffer, description);
	}

	/// <inheritdoc />
	public override TopLevelAS BuildRaytracingAccelerationStructure(in TopLevelASDescription description)
	{
		return new VKTopLevelAS(context, CommandBuffer, description);
	}

	/// <inheritdoc />
	public override void UpdateRaytracingAccelerationStructure(ref TopLevelAS tlas, in TopLevelASDescription newDescription)
	{
		((VKTopLevelAS)tlas).UpdateAccelerationStructure(CommandBuffer, newDescription);
	}

	/// <inheritdoc />
	public override void DispatchRays(in DispatchRaysDescription description)
	{
		...
		VkStridedDeviceAddressRegionKHR rayGenSBT = VkStridedDeviceAddressRegionKHR()
			{
				deviceAddress = shaderBindingTable.GetRayGenStartAddress(),
				stride = shaderBindingTable.GetRayGenStride(),
				size = shaderBindingTable.GetRayGenSize()
			};
		VkStridedDeviceAddressRegionKHR rayMissSBT = VkStridedDeviceAddressRegionKHR()
			{
				deviceAddress = shaderBindingTable.GetMissStartAddress(),
				stride = shaderBindingTable.GetMissStride(),
				size = shaderBindingTable.GetMissSize()
			};
		VkStridedDeviceAddressRegionKHR rayHitSBT = VkStridedDeviceAddressRegionKHR()
			{
				deviceAddress = shaderBindingTable.GetHitGroupStartAddress(),
				stride = shaderBindingTable.GetHitGroupStride(),
				size = shaderBindingTable.GetHitGroupSize()
			};
	...
	/// <inheritdoc />
	public override void DispatchMesh(uint32 groupCountX, uint32 groupCountY, uint32 groupCountZ)
	{
		VulkanNative.vkCmdDrawMeshTasksEXT(CommandBuffer, groupCountX, groupCountY, groupCountZ);
	}
