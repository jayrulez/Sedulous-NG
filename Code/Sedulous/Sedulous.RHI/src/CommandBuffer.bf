...
public abstract class CommandBuffer : IDisposable
{
	...
	public void SetVertexBuffers(Buffer[] buffers, uint32[] offsets)
	{
	...
	/// <param name="buffers">The array of vertex buffers being bound.</param>
	/// <param name="offsets">Offsets (in bytes) from the start of each vertex buffer to the first vertex to use.</param>
	protected abstract void SetVertexBuffersInternal(Buffer[] buffers, uint32[] offsets);
	...
	/// <summary>
	/// Sets the mesh‐shader pipeline state object for this command buffer.
	/// </summary>
	/// <param name="pipeline">The mesh shader pipeline state description.</param>
	public void SetMeshShaderPipelineState(MeshShaderPipelineState pipeline)
	{
		SetMeshShaderPipelineStateInternal(pipeline);
	}

	/// <summary>
	/// Sets the mesh‐shader pipeline state object for this command buffer.
	/// Concrete implementations must bind the provided mesh shader PSO.
	/// </summary>
	/// <param name="pipeline">The mesh shader pipeline state description.</param>
	protected abstract void SetMeshShaderPipelineStateInternal(MeshShaderPipelineState pipeline);
	...
	public void UpdateBufferData<T>(Buffer buffer, T[] data, uint32 count = 0, uint32 destinationOffsetInBytes = 0) where T : struct
	{
		uint32 dataSizeInBytes = (uint32)(((count != 0) ? ((int32)count) : ((data != null) ? data.Count : 0)) * sizeof(T));
	...
	/// </summary>
	/// <param name="source">Source multisampled texture.</param>
	/// <param name="destination">Non-multisampled texture.</param>
	...
	public abstract BottomLevelAS BuildRaytracingAccelerationStructure(in BottomLevelASDescription blas);
	...
	public abstract TopLevelAS BuildRaytracingAccelerationStructure(in TopLevelASDescription tlas);
	...
	public abstract void UpdateRaytracingAccelerationStructure(ref TopLevelAS tlas, in TopLevelASDescription newDescription);
	...
	public abstract void DispatchRays(in DispatchRaysDescription description);

	/// <summary>
	/// Issues a GPU command to launch the mesh shader pipeline using the specified thread group dimensions.
	/// This method replaces the traditional vertex (VS) and geometry (GS) shader stages, enabling flexible and
	/// parallel mesh generation directly on the GPU.
	/// </summary>
	/// <param name="groupCountX">The number of thread groups to dispatch in the X dimension.
	/// Each group executes one instance of the mesh shader (or the amplification shader if used).</param>
	/// <param name="groupCountY">The number of thread groups to dispatch in the Y dimension.</param>
	/// <param name="groupCountZ">The number of thread groups to dispatch in the Z dimension.</param>
	public abstract void DispatchMesh(uint32 groupCountX, uint32 groupCountY, uint32 groupCountZ);
