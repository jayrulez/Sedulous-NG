...
public class VKGraphicsContext : GraphicsContext
{
	...
	internal struct VKPhysicalDeviceInfo
	{
		...
		internal VkPhysicalDeviceMeshShaderFeaturesEXT MeshShaderFeatures;
	}
	...
	internal const uint32 Version_1_4 = VKHelpers.Version(1, 4, 0);

	internal const uint32 Version_1_3 = VKHelpers.Version(1, 3, 0);

	internal const uint32 Version_1_2 = VKHelpers.Version(1, 2, 0);

	internal const uint32 Version_1_1 = VKHelpers.Version(1, 1, 0);

	internal const uint32 Version_1_0 = VKHelpers.Version(1, 0, 0);
	...
	internal bool meshshaderSupported;
	...
	private List<String> DebugInstanceFunctions = new .() ~ delete _;
	...
	public this(ILogger logger)
		: this(logger, Span<String>(), Span<String>())
	{
	}
	...
	public this(ILogger logger, Span<String> deviceExtensionsToEnable, Span<String> instanceExtensionsToEnable)
		: base(logger)
	{
		...
		CreateInstance(scope (instance) =>
			{
				...
				VulkanNative.LoadInstanceFunctions(instance, flags, DebugInstanceFunctions).ReturnValueDiscarded();
	...
	{
		if (let impl = swapChain as VKSwapChain)
		{
			impl.Dispose();
			delete impl;
			swapChain = null;
	...
	public override MappedResource MapMemory(GraphicsResource resource, MapMode mode, uint32 subResource = 0)
	{
		...
		if (resource is VKTexture)
		{
			...
			if ((texture.Description.Usage & ResourceUsage.Staging) != ResourceUsage.Default)
			{
				void* dataPointer = default(void*);
	...
	private void CreateInstance(delegate void(VkInstance) onInstanceCreated = null)
	{
		...
		if (base.IsValidationLayerEnabled)
		{
			if (availableInstanceExtensions.Contains("VK_EXT_debug_utils"))
			{
				...
				DebugInstanceFunctions.AddRange(scope List<String>()
					{
			...
			}else if (availableInstanceExtensions.Contains("VK_EXT_debug_report"))
			{
		...
		VkInstanceCreateInfo instanceInfo = VkInstanceCreateInfo()
			{
				...
				enabledLayerCount = (uint32)layersToEnable.Count,
				ppEnabledLayerNames = layersToEnableArray,
				enabledExtensionCount = (uint32)extensionsCount,
	...
	private void CreatePhysicalAndLogicalDevice()
	{
		...
		VkPhysicalDeviceProperties2 deviceProperties2 = VkPhysicalDeviceProperties2()
			{
		...
		List<uint32> uniqueQueueFamilies = scope List<uint32>() { QueueIndices.GraphicsFamily };
		if (QueueIndices.CopyQueueSupported)
		{
			uniqueQueueFamilies.Add(QueueIndices.CopyFamily);
		}
		if (QueueIndices.ComputeFamilySupported)
		{
			uniqueQueueFamilies.Add(QueueIndices.ComputeFamily);
		}
		float priority = 1f;
		VkDeviceQueueCreateInfo* queueCreateInfos = scope VkDeviceQueueCreateInfo[uniqueQueueFamilies.Count]*;
		for (int32 j = 0; j < uniqueQueueFamilies.Count; j++)
		{
			queueCreateInfos[j] = VkDeviceQueueCreateInfo()
				{
					sType = VkStructureType.VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
					queueFamilyIndex = uniqueQueueFamilies[j],
					queueCount = 1,
		...
		VkPhysicalDeviceFeatures2 deviceFeatures2 = VkPhysicalDeviceFeatures2()
			{
		...
		VkPhysicalDeviceMeshShaderFeaturesEXT meshShaderFeatures = default(VkPhysicalDeviceMeshShaderFeaturesEXT);
		if (deviceProperties2.properties.apiVersion >= Version_1_1)
		{
			...
			if (deviceProperties2.properties.apiVersion >= Version_1_2)
			{
				...
				if (deviceProperties2.properties.apiVersion >= Version_1_3)
				{
					...
					{
						*features_chain = &accelerationStructureFeatures;
						...
						features_chain = &raytracingFeatures.pNext;
					}
					if (meshshaderSupported)
					{
						*features_chain = &meshShaderFeatures;
						meshShaderFeatures.sType = VkStructureType.VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MESH_SHADER_FEATURES_EXT;
		...
		VkDeviceCreateInfo createInfo = VkDeviceCreateInfo()
			{
				...
				enabledExtensionCount = (uint32)extensionsCount,
				ppEnabledExtensionNames = extensionsToEnableArray,
				queueCreateInfoCount = (uint32)uniqueQueueFamilies.Count,
		...
		VulkanNative.vkGetDeviceQueue(VkDevice, QueueIndices.GraphicsFamily, 0, &newQueue);
		vkGraphicsQueue = newQueue;
	}

	private void CreateResourcesForCopyQueue()
	{
		uint32 queueFamilyIndex = (QueueIndices.CopyQueueSupported ? QueueIndices.CopyFamily : 0);
		VkQueue newQueue = default(VkQueue);
		...
		VkCommandBufferAllocateInfo commandBufferInfo = VkCommandBufferAllocateInfo()
			{
	...
	internal void SetDebugName(VkObjectType type, uint64 target, String name)
	{
		...
		{
			VkDebugUtilsObjectNameInfoEXT nameInfo = VkDebugUtilsObjectNameInfoEXT()
				{
					sType = VkStructureType.VK_STRUCTURE_TYPE_DEBUG_UTILS_OBJECT_NAME_INFO_EXT,
					objectHandle = target,
					objectType = type,
					pObjectName = scope String(name).CStr()
				};
