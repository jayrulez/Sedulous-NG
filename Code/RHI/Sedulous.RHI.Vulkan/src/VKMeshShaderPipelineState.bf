...
/// </summary>
public class VKMeshShaderPipelineState : MeshShaderPipelineState
{
	/// <summary>
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="T:Sedulous.RHI.Vulkan.VKMeshShaderPipelineState" /> class.
	/// </summary>
	/// <param name="context">The graphics context.</param>
	/// <param name="description">The graphics pipeline state description.</param>
	public this(VKGraphicsContext context, in MeshShaderPipelineDescription description)
		: base(description)
	{
		vkContext = context;
		...
		for (int32 i = 0; i < renderTargetCount; i++)
		{
		...
		*dynamicStates = VkDynamicState.VK_DYNAMIC_STATE_VIEWPORT;
		...
		if (description.Shaders.AmplificationShader != null)
		{
			VKShader amplificationShader = description.Shaders.AmplificationShader as VKShader;
			stages.Add(amplificationShader.ShaderStateInfo);
		}
		if (description.Shaders.MeshShader != null)
		{
			VKShader meshShader = description.Shaders.MeshShader as VKShader;
			stages.Add(meshShader.ShaderStateInfo);
		}
		...
		for (int32 j = 0; j < stages.Count; j++)
		{
			stagePointer[j] = stages[j];
		}
		...
		if (description.ResourceLayouts != null)
		{
			...
			for (int32 k = 0; k < description.ResourceLayouts.Count; k++)
			{
				VKResourceLayout layout = description.ResourceLayouts[k] as VKResourceLayout;
				layouts[k] = layout.DescriptorSetLayout;
			}
	...
	private VkRenderPass CreateCompatibilityRenderPass(OutputDescription outputs)
	{
		...
		if (outputs.DepthAttachment.HasValue)
		{
			...
			depthAttachementReference = VkAttachmentReference()
				{
					attachment = currentAttachmentIndex,
			...
			if (depthAttachment.ResolveMSAA)
			{
				...
				VkAttachmentReference vkAttachmentReference = VkAttachmentReference()
					{
						attachment = currentAttachmentIndex,
						layout = VkImageLayout.VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
					};
		...
		VkRenderPassCreateInfo renderPassInfo = VkRenderPassCreateInfo()
			{
				...
				attachmentCount = currentAttachmentIndex,
