...
public class ValidationLayer
{
	...
	/// <summary>
	/// Creates a mesh shader pipeline validation layer.
	/// </summary>
	/// <param name="description">The mesh shader pipeline description.</param>
	public void CreateMeshShaderPipelineValidation(in MeshShaderPipelineDescription description)
	{
	}
	...
	public void CreateBufferValidation(void* data, in BufferDescription description)
	{
		...
		if ((description.Flags & BufferFlags.ConstantBuffer) != BufferFlags.None && description.SizeInBytes % 16 != 0)
		{
	...
	internal void SetVertexBuffers(bool inRenderPass, Buffer[] buffers, uint32[] offsets)
	{
		if (!inRenderPass)
		{
	...
	{
		mLogger.LogTrace(scope $"A {owner} error was encountered: {message}");
	}
	...
	private void NotifyException(String owner, String message, Severity severity)
	{
		...
		case Severity.Information:
			mLogger.LogInformation(fullMessage);
			break;
		case Severity.Warning:
			mLogger.LogWarning(fullMessage);
			break;
		case Severity.Error:
