...
/// </summary>
public sealed class SDL3GamePadDevice : GamePadDevice
{
	/// <summary>
	...
	public this(SDL3InputSystem inputSystem, uint32 joystickIndex, int32 playerIndex)
		: base(inputSystem)
	{
		...
		var joystick = SDL_GetGamepadJoystick(controller);
	...
	internal bool HandleEvent(SDL_Event evt)
	{
		...
		case .SDL_EVENT_GAMEPAD_BUTTON_DOWN:
			{
				if (evt.gbutton.which == instanceID)
				{
					if (!isRegistered)
						Register();

					var button = SDLToFrameworkButton((SDL_GamepadButton)evt.gbutton.button);
		...
		case .SDL_EVENT_GAMEPAD_AXIS_MOTION:
			{
				if (evt.gaxis.which == instanceID)
				{
					if (!isRegistered)
						Register();

					OnAxisMotion(evt.gaxis);
				}
			}
			return true;
...
#unwarn
	...
	/// </summary>
	internal uint32 InstanceID => instanceID;
	...
	private static GamePadButton SDLToFrameworkButton(SDL_GamepadButton button) => (GamePadButton)(1 + (int)button);
	...
	private void OnAxisMotion(SDL_GamepadAxisEvent evt)
	{
	...
	private readonly SDL_Gamepad* controller;
