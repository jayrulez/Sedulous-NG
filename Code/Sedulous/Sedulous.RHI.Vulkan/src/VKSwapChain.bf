...
public class VKSwapChain : SwapChain
{
	...
	private void CreateSurface()
	{
		...
		case PlatformType.Windows:
			{
				VkWin32SurfaceCreateInfoKHR windowsSurfaceInfo = VkWin32SurfaceCreateInfoKHR()
					{
						sType = VkStructureType.VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR,
						hwnd = base.SwapChainDescription.SurfaceInfo.Win32.Hwnd,
		...
		case PlatformType.Linux:
			...
			{
				VkWaylandSurfaceCreateInfoKHR wayLandSurfaceInfo = VkWaylandSurfaceCreateInfoKHR()
					{
						sType = VkStructureType.VK_STRUCTURE_TYPE_WAYLAND_SURFACE_CREATE_INFO_KHR,
						display = base.SwapChainDescription.SurfaceInfo.Wayland.Display,
						surface = base.SwapChainDescription.SurfaceInfo.Wayland.Surface
					};
				VulkanNative.vkCreateWaylandSurfaceKHR(vkContext.VkInstance, &wayLandSurfaceInfo, null, &newSurface);
			}
			else
			{
				VkXlibSurfaceCreateInfoKHR x11SurfaceInfo = VkXlibSurfaceCreateInfoKHR()
					{
						sType = VkStructureType.VK_STRUCTURE_TYPE_XLIB_SURFACE_CREATE_INFO_KHR,
						dpy = base.SwapChainDescription.SurfaceInfo.X11.Display,
						window = (void*)(int)base.SwapChainDescription.SurfaceInfo.X11.Window
					};
		...
		case PlatformType.Android:
			{
				...
				VkAndroidSurfaceCreateInfoKHR androidSurfaceInfo = VkAndroidSurfaceCreateInfoKHR()
				{
		...
		case PlatformType.MacOS:
			{
				VkMacOSSurfaceCreateInfoMVK macOSSurfaceInfo = VkMacOSSurfaceCreateInfoMVK()
					{
						sType = VkStructureType.VK_STRUCTURE_TYPE_MACOS_SURFACE_CREATE_INFO_MVK,
						pView = (void*)base.SwapChainDescription.SurfaceInfo.MetalMacOS.CAMetalLayer
					};
		...
		case PlatformType.iOS:
			{
				VkIOSSurfaceCreateInfoMVK iOSSurfaceInfo = VkIOSSurfaceCreateInfoMVK()
					{
						sType = VkStructureType.VK_STRUCTURE_TYPE_IOS_SURFACE_CREATE_INFO_MVK,
						pView = (void*)base.SwapChainDescription.SurfaceInfo.MetalIOS.View
					};
	...
	private void CreateSwapChain()
	{
		...
		VkSwapchainCreateInfoKHR vkSwapchainInfo = VkSwapchainCreateInfoKHR()
			{
		...
		uint32 graphicsQueueIndex = vkContext.QueueIndices.GraphicsFamily;
		uint32 presentQueueIndex = vkContext.QueueIndices.Presentfamily;
		uint32* queueFamilices = scope uint32[2]* (graphicsQueueIndex, presentQueueIndex);
		if (vkContext.QueueIndices.PresentFamilySupported && graphicsQueueIndex != presentQueueIndex)
		{
	...
	private void DestroySwapChain()
	{
		if (vkSwapChain != VkSwapchainKHR.Null)
		{
			...
			if (base.FrameBuffer != null)
			{
				vkContext.Factory.DestroyFrameBuffer(ref base.FrameBuffer);
			}
		}
	}
