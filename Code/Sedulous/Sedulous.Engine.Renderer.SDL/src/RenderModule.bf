...
class RenderModule : SceneModule
{
	...
	private SDL_GPUSampler* mDefaultDepthSampler => mRenderer.GetDefaultDepthSampler();

	// Shadow mapping
	private struct ShadowMapData
	{
	...
	private Dictionary<Light, ShadowMapData> mShadowMaps = new .() ~ delete _;
	...
	public ~this()
	{
		...
		for (var entry in mShadowMaps)
		{
			SDL_ReleaseGPUTexture(mRenderer.mDevice, entry.value.Texture);
			SDL_ReleaseGPUSampler(mRenderer.mDevice, entry.value.Sampler);
		}
	...
	internal void RenderFrame()
	{
		...
		RenderShadowMaps(commandBuffer);
		...
		// Second pass: 3D objects with depth and shadows
		Render3DObjects(commandBuffer, swapchainTexture);

		// Third pass: Sprites (no shadows)
	...
	private void RenderShadowMaps(SDL_GPUCommandBuffer* commandBuffer)
	{
		// Render shadow map for each shadow-casting light
		int lightIndex = 0;
		for (var (light, transform) in mLights)
		{
			if (!light.CastShadows || lightIndex >= MAX_LIGHTS)
				break;

			// Create or update shadow map for this light
			if (!mShadowMaps.ContainsKey(light))
			{
				CreateShadowMapForLight(light);
			}

			var shadowData = mShadowMaps[light];

			// Calculate light space matrix
			Matrix lightView, lightProjection;
			CalculateLightMatrices(light, transform, out lightView, out lightProjection);
			Matrix lightSpaceMatrix = lightView * lightProjection;

			// Render to shadow map
			RenderShadowMapForLight(commandBuffer, shadowData, lightSpaceMatrix);

			lightIndex++;
		}
	}

	private void CreateShadowMapForLight(Light light)
	{
		var shadowMapDesc = SDL_GPUTextureCreateInfo()
			{
				type = .SDL_GPU_TEXTURETYPE_2D,
				format = .SDL_GPU_TEXTUREFORMAT_D32_FLOAT,
				width = (uint32)light.ShadowMapSize,
				height = (uint32)light.ShadowMapSize,
				layer_count_or_depth = 1,
				num_levels = 1,
				sample_count = .SDL_GPU_SAMPLECOUNT_1,
		...
		var texture = SDL_CreateGPUTexture(mRenderer.mDevice, &shadowMapDesc);
		...
		var sampler = SDL_CreateGPUSampler(mRenderer.mDevice, &samplerDesc);

		mShadowMaps[light] = ShadowMapData()
			{
				Texture = texture,
				Sampler = sampler,
	...
	private void CalculateLightMatrices(Light light, Transform transform, out Matrix view, out Matrix projection)
	{
		...
		{
			var dirLight = light as DirectionalLight;

			// For directional lights, position the light based on the camera
			if (mActiveCamera != null)
			{
				position = mActiveCameraTransform.Position - forward * dirLight.ShadowDistance * 0.5f;
			}
			...
			float orthoSize = dirLight.ShadowOrthoSize;
			projection = Matrix.CreateOrthographic(orthoSize, orthoSize, 0.1f, dirLight.ShadowDistance);
		}
		else if (light is SpotLight)
		{
			...
			float fov = Math.DegreesToRadians(spotLight.OuterConeAngle * 2.0f);
			projection = Matrix.CreatePerspectiveFieldOfView(fov, 1.0f, 0.1f, spotLight.Range);
		}
		else if (light is PointLight)
		{
			// Point lights would need cube map shadows (6 passes)
			// For now, we'll skip point light shadows
	...
	private void RenderShadowMapForLight(SDL_GPUCommandBuffer* commandBuffer, ShadowMapData shadowData, Matrix lightSpaceMatrix)
	{
		// Setup shadow render pass
		...
		var renderPass = SDL_BeginGPURenderPass(commandBuffer, null, 0, &depthTarget);
		...
		var viewport = SDL_GPUViewport()
			{
				x = 0, y = 0,
				w = (float)shadowData.Size,
				h = (float)shadowData.Size,
				min_depth = 0.0f,
				max_depth = 1.0f
			};
		...
		SDL_BindGPUGraphicsPipeline(renderPass, mRenderer.GetShadowPipeline());

		// Render all mesh objects
		for (var command in mMeshCommands)
		{
			RenderObjectShadow(commandBuffer, renderPass, command, lightSpaceMatrix);
		}
	}

	private void RenderObjectShadow(SDL_GPUCommandBuffer* commandBuffer, SDL_GPURenderPass* renderPass,
		MeshRenderCommand command, Matrix lightSpaceMatrix)
	{
		if (!mRendererMeshes.ContainsKey(command.Renderer))
			return;
		...
		SDL_BindGPUIndexBuffer(renderPass, scope .() { buffer = mesh.IndexBuffer, offset = 0 }, .SDL_GPU_INDEXELEMENTSIZE_32BIT);

		// Push light space matrix
		var uniformData = command.WorldMatrix * lightSpaceMatrix;
		SDL_PushGPUVertexUniformData(commandBuffer, 0, &uniformData, sizeof(Matrix));
	...
	private void RenderObject(SDL_GPUCommandBuffer* commandBuffer, SDL_GPURenderPass* renderPass, MeshRenderCommand command)
	{
		...
		// Bind shadow maps - ALWAYS bind something to each slot
		for (int i = 0; i < MAX_LIGHTS; i++)
		{
		    ...
		    if (i < mLights.Count)
		    {
		        var (light, _) = mLights[i];
		        if (light.CastShadows && mShadowMaps.TryGetValue(light, let shadowData))
		        {
		            shadowBinding = SDL_GPUTextureSamplerBinding()
		            {
		                texture = shadowData.Texture,
		                sampler = shadowData.Sampler
		            };
		        }
		        else
		        {
		            // Bind default shadow texture when no shadow map exists
		            shadowBinding = SDL_GPUTextureSamplerBinding()
		            {
		                texture = mRenderer.GetDefaultShadowTexture(),
		                sampler = mRenderer.GetDefaultShadowSampler()
		            };
		        }
		    }
		    else
		    {
		        // No light at this index - bind default shadow texture
		        shadowBinding = SDL_GPUTextureSamplerBinding()
		        {
		            texture = mRenderer.GetDefaultShadowTexture(),
		            sampler = mRenderer.GetDefaultShadowSampler()
		    ...
		    SDL_BindGPUFragmentSamplers(renderPass, (uint32)(usePBR ? 3 + i : 1 + i), &shadowBinding, 1);
		}
		...
		if (mActiveCamera != null)
		{
			if (usePBR)
			{
				...
				for (int i = 0; i < lightCount; i++)
				{
					...
					fragmentUniforms.Lights[i].SpotAngles = Vector4(innerCos, outerCos, light.ShadowBias, light.ShadowNormalBias);

					// Calculate and set shadow matrix
					if (light.CastShadows && mShadowMaps.ContainsKey(light))
					{
						Matrix lightView, lightProjection;
						CalculateLightMatrices(light, transform, out lightView, out lightProjection);
						fragmentUniforms.Lights[i].ShadowMatrix = command.WorldMatrix * lightView * lightProjection;
					}
					else
					{
						fragmentUniforms.Lights[i].ShadowMatrix = Matrix.Identity;
					}
				}
