		...
		VkBufferCreateInfo bufferInfo = VkBufferCreateInfo()
			{
				...
				sharingMode = (nativeContext.QueueIndices.CopyQueueSupported ? VkSharingMode.VK_SHARING_MODE_CONCURRENT : VkSharingMode.VK_SHARING_MODE_EXCLUSIVE)
			};
		int32 queueFamilies = ((!nativeContext.QueueIndices.CopyQueueSupported) ? 1 : 2);
		uint32* queueFamilyIndices = scope uint32[queueFamilies]*;
		*queueFamilyIndices = nativeContext.QueueIndices.GraphicsFamily;
		if (nativeContext.QueueIndices.CopyQueueSupported)
		{
			queueFamilyIndices[1] = nativeContext.QueueIndices.CopyFamily;
		}
